...
## 3.1. Trial and error  
##### (1) 데이터셋 변경  
본 프로젝트는 초기에 '평상시'와 '블랙프라이데이' 시즌 간의 소비자 행동 변화를 비교 분석하고자 함. 그러나 초기데이터 탐색 및 시각화 과정에서 기존 데이터셋의 한계점을 발견하여 프로젝트 방향을 수정하게 됨.  
...
위 세 가지 분석 결과로, 해당 데이터셋은 실제 소비자의 역동적인 구매 이력을 기록한 데이터가 아닌, **학습 및 예측 모델링 연습을 위해 통계적 패키지로 무작위 생성된 가상 데이터**인 것으로 판단되었고, 인구통계학적으로 데이터셋을 분석하기에 부적합하다고 판단하여 현재의 데이터셋으로 변경하였음.
...
<br>
#6.
그 다음으로 https://www.kaggle.com/datasets/ibikunlegabriel/christmas-sales-and-trends에서 사용한 데이터셋을 사용해 분석하고자 했다. 해당 데이터셋은 11월,12월의 데이터가 크리스마스 기간, 블랙프라이데이 기간, 그 외의 기간이 포함되어 있었다. 해당 데이터에서 크리스마스 기간을 제외해 일반적인 기간과 블랙프라이 데이 기간을 비교분석하고자 했다. 해당 데이터셋에 나이, 성별, 구매 가격, 할인 가격, 판매일 등이 기재되어 있었다. <img width="1645" height="943" alt="image" src="https://github.com/user-attachments/assets/4aaf9260-4bcc-45a5-a94f-460a8c88c8c8" />
 해당 가상 데이터를 사용한 분석을 통해 카테고리 별 매출 분포, 건당 매출 평균액 등 유의미한 데이터 차이가 있었다. 그러나 실제 데이터셋을 보자 모든 상품의 할인율이 10%로 동일하였다. 할인 여부와 상관없이 모든 상품에서 같은 할인율지니고 있어 할인금액을 통한 그래프로는 유의미한 데이터를 얻을 수 없었다. 또한 해당 데이터셋에서 블랙프라이데이나 크리스마스를 제외한 날들은 모두 할인이 없는 것으로 정하였다. 이는 실제 데이터값과 차이가 있을 것이라 생각해 더 나은 데이터셋을 찾기로 하였다
