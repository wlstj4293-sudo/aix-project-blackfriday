...
[실험 1]과 [실험]2에서 걸리는 시간(자원)은 Deep DNN> Wide NN ≈ Hist GB > Random Forest 순서였다. 해당 이유를 찾아본 결과
Deep DNN은 128→64→32→1로 레이어가 4개이고. 각 레이어마다 행렬 곱셈 + BatchNorm + ReLU를 30 에폭(전체 학습 데이터셋을 모델이 처음부터 끝까지 완전히 한 번 학습하는 과정) × 여러 배치만큼 반복한. 레이어가 많을수록 순전파와 역전파연산량이 누적되어 가장 오래 걸렸다. 또한 Wide NN과 Hist GB에서 HistGB는 "트리 개수 × 직렬 순차"의 연산을 해야 해서 느리고, WideNN은 "512차원 행렬 연산 × 30에폭"연산을 해야 하는 각자 다른 이유로 0.6초대에 수렴한 것이다. Random Forest는 여러개의 트리를 한번에 학습하기 때문에 속도가 제일 빠르게 나왔다.
---
